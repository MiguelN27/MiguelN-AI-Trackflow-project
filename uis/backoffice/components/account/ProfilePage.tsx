...
export default function ProfilePage() {
  ...
  return (
    <main className="mx-auto flex w-full max-w-3xl flex-col px-6 py-10 md:px-10 md:py-14">
      <header>
        <p className="font-mono text-xs uppercase tracking-[0.22em] text-[color:var(--brand-primary)]">Account</p>
        <h1 className="mt-3 text-3xl font-semibold text-[color:var(--foreground)] md:text-4xl">Your profile</h1>
      ...
      <section className="mt-8 rounded-3xl border border-[color:var(--border-soft)] bg-[color:var(--surface)]/90 p-6 shadow-[0_24px_90px_rgba(37,99,235,0.1)] backdrop-blur md:p-8">
        <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Account</h2>
        <dl className="mt-4 grid gap-4 sm:grid-cols-2">
          <div>
            <dt className="text-xs uppercase tracking-[0.14em] text-slate-500">Email</dt>
            <dd className="mt-1 font-mono text-sm text-[color:var(--foreground)]">{user?.email ?? "-"}</dd>
          </div>
          <div>
            <dt className="text-xs uppercase tracking-[0.14em] text-slate-500">Role</dt>
            <dd className="mt-1 text-sm capitalize text-[color:var(--foreground)]">{user?.role ?? "-"}</dd>
          </div>
        </dl>
      ...
      <section className="mt-6 rounded-3xl border border-[color:var(--border-soft)] bg-[color:var(--surface)]/90 p-6 shadow-[0_24px_90px_rgba(37,99,235,0.1)] backdrop-blur md:p-8">
        <h2 className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Profile</h2>
        ...
        {profileId ? (
          <form className="mt-4 grid gap-4" onSubmit={handleSubmit} noValidate>
            ...
            <p className="text-xs text-slate-500">Clearing a field removes it from your profile.</p>
